...
| `label`      | `label`       | Accessible name for the dialog, set as `aria-label`. A slotted heading cannot be referenced with `aria-labelledby` across the shadow boundary, so the name comes through this prop.          | `string`  | `undefined` |
| `open`       | `open`        | Whether the dialog is open. Kept in sync when the user closes it with `Escape` or the close button.                                                                                          | `boolean` | `false`     |
