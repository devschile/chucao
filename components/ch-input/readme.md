...
| `errorMessage` | `error-message` | Error message rendered below the input when it is invalid.                                                                           | `string`  | `undefined` |
| `hint`         | `hint`          | Helper text rendered below the input, associated via `aria-describedby`. Replaced by `errorMessage` when the input is invalid.       | `string`  | `undefined` |
| `invalid`      | `invalid`       | Whether the input is in an invalid state; toggles `aria-invalid` and error styling.                                                  | `boolean` | `false`     |
